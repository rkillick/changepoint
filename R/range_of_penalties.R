...
range_of_penalties <-
  function(sumstat,
           cost = "mean.norm",
           PELT = T,
           min_pen = log(length(sumstat) / 3 - 1),
           max_pen = 10 * log(length(sumstat) / 3 - 1),
           shape = 1,
           minseglen) {
    NCALC = 0
    ...
    while (length(pen_interval) > 0) {
      ...
      for (b in 1:length(pen_interval)) {
        ...
        new_penalty[b] <-
          ans[[3]][n + 1] - (ans[[4]][n + 1] - 1) * pen_interval[b]
      }
      
      if (count == 0) {
        print(
          paste(
            "Maximum number of runs of algorithm = ",
            new_numcpts[1] - new_numcpts[2] + 2,
            sep = ""
          )
        )
      ...
      test_penalties <- unique((sort(c(
        test_penalties, pen_interval
      ))))
      ...
      numberofchangepoints <-
        -sort(-new_numcpts) ##can use sort to re-order
      ...
      for (i in 1:(length(test_penalties) - 1)) {
        if (abs(numberofchangepoints[i] - numberofchangepoints[i + 1]) > 1) {
          ##only need to add a beta if difference in cpts>1
          j <- i + 1
          tmppen_interval <-
            (penal[j] - penal[i]) * (((numberofchangepoints[i]) - (numberofchangepoints[j])) ^
                                       -1)
