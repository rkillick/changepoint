single.meanvar.gamma.calc <-
  function(data,
    ...
    singledim = function(data, shape, extrainf = TRUE, minseglen) {
      ...
      tmp = 2 * taustar * shape * log(y[taustar + 1]) - 2 * taustar * shape *
        log(taustar * shape) + 2 * (n - taustar) * shape * log((y[n + 1] - y[taustar +
                                                                               1])) - 2 * (n - taustar) * shape * log((n - taustar) * shape)
...
single.meanvar.gamma <-
  function(data,
           ...
           minseglen) {
    if (sum(data <= 0) > 0) {
      stop('Gamma test statistic requires positive data')
    }
    if (is.null(dim(data)) == TRUE) {
      # single dataset
      n = length(data)
...
multiple.meanvar.gamma = function(data,
                                  shape = 1,
                                  mul.method = "PELT",
                                  penalty = "MBIC",
                                  pen.value = 0,
  ...
  pen.value = penalty_decision(penalty,
                               pen.value,
                               n,
                               diffparam,
                               asymcheck = costfunc,
