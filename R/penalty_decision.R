penalty_decision = function(penalty,
                            pen.value,
                            n,
                            diffparam,
                            asymcheck,
  ...
  if (penalty == "Asymptotic") {
    ...
    if (asymcheck == "mean.norm") {
      ...
      pen.return = (-alogn * log(log((
        1 - alpha + exp(-2 * (pi ^ (1 / 2)) * exp(blogn / alogn))
      ) ^ (-1 / (
        2 * (pi ^ (1 / 2))
      )))) + blogn) ^ 2
      ...
      pen.return = (-(log(log((1 - alpha + exp(-2 * exp(blogn))) ^ (-1 /
                                                                      2)
      ))) / alogn + blogn / alogn) ^ 2
      ...
      top = -(log(log((
        1 - alpha + exp(-2 * exp(
          2 * (log(log(n))) + (diffparam / 2) * (log(log(log(
            n
          )))) - log(gamma(diffparam / 2))
        ))
      ) ^ (-1 / 2))))  +  2 * (log(log(n))) + (diffparam / 2) * (log(log(log(n)))) - log(gamma(diffparam /
                                                                                                 2))
      ...
      else if (pen.value == 0.05) {
        pen.return = 1.358
      }
      else if (pen.value == 0.1) {
        pen.return = 1.224
      }
      else if (pen.value == 0.25) {
        pen.return = 1.019
      }
      else if (pen.value == 0.5) {
        pen.return = 0.828
      }
      else if (pen.value == 0.75) {
        pen.return = 0.677
      }
      else if (pen.value == 0.9) {
        pen.return = 0.571
      }
      else if (pen.value == 0.95) {
        pen.return = 0.520
      }
      else{
        stop('Only alpha values of 0.01,0.05,0.1,0.25,0.5,0.75,0.9,0.95 are valid for CSS')
      }
      ...
      stop(
        'Asymptotic penalties for the Gamma test statistic are not defined, please choose an alternative penalty type'
      )
      ...
      if (alpha == 1) {
        pen.return = 1.42417
      } # value of 1 gives log(0), this is alpha=0.99999999999999993
    } else if (asymcheck == "meanvar.poisson") {
      stop(
        'Asymptotic penalties for the Poisson test statistic are not available yet, please choose an alternative penalty type'
      )
    }
  }
  #if(method=="AMOC"){
