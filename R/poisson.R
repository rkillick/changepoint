single.meanvar.poisson.calc <-
  function(data, extrainf = TRUE, minseglen) {
    singledim = function(data, extrainf = TRUE, minseglen) {
      ...
      tmp = 2 * log(taustar) * y[taustar + 1] - 2 * y[taustar + 1] * log(y[taustar +
                                                                             1]) + 2 * log(n - taustar) * (y[n + 1] - y[taustar + 1]) - 2 * (y[n + 1] -
                                                                                                                                               y[taustar + 1]) * log((y[n + 1] - y[taustar + 1]))
      if (sum(is.na(tmp)) != 0) {
...
single.meanvar.poisson <-
  function(data,
           ...
           minseglen) {
    if ((sum(data < 0) > 0)) {
      stop('Poisson test statistic requires positive data')
    }
    if (sum(as.integer(data) == data) != length(data)) {
      stop('Poisson test statistic requires integer data')
    }
    if (is.null(dim(data)) == TRUE) {
      # single dataset
      n = length(data)
...
multiple.meanvar.poisson = function(data,
                                    mul.method = "PELT",
                                    penalty = "MBIC",
                                    pen.value = 0,
                                    Q = 5,
