...
cpt.meanvar = function(data,
  ...
  if (minseglen < 2) {
    if (!(minseglen == 1 &
          (test.stat == "Poisson" | test.stat == "Exponential"))) {
      minseglen = 2
      warning(
        'Minimum segment length for a change in mean and variance is 2, automatically changed to be 2.'
      )
    }
  }
  if (penalty == "CROPS") {
...
checkData = function(data) {
  ...
  if (anyNA(data)) {
    stop(
      "Missing value: NA is not allowed in the data as changepoint methods assume regularly spaced data."
    )
  }
