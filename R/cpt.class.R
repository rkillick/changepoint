setClass(
  ...
  prototype = prototype(
    cpttype = "Not Set",
    date = date(),
    version = as(packageVersion("changepoint"), 'character')
  )
)
...
setMethod("data.set", "cpt", function(object)
  coredata(object@data.set))
setMethod("data.set", "cpt.reg", function(object)
  coredata(object@data.set))
...
setMethod("data.set.ts", "cpt", function(object)
  object@data.set)
...
setMethod("cpttype", "cpt", function(object)
  object@cpttype)
setMethod("cpttype", "cpt.reg", function(object)
  object@cpttype)
...
setMethod("method", "cpt", function(object)
  object@method)
setMethod("method", "cpt.reg", function(object)
  object@method)
...
setMethod("distribution", "cpt", function(object)
  object@test.stat)
setMethod("distribution", "cpt.reg", function(object)
  object@test.stat)
...
setMethod("test.stat", "cpt", function(object)
  object@test.stat)
setMethod("test.stat", "cpt.reg", function(object)
  object@test.stat)
...
setMethod("pen.type", "cpt", function(object)
  object@pen.type)
setMethod("pen.type", "cpt.reg", function(object)
  object@pen.type)
...
setMethod("pen.value", "cpt", function(object)
  object@pen.value)
setMethod("pen.value", "cpt.reg", function(object)
  object@pen.value)
...
setMethod("pen.value.full", "cpt.range", function(object)
  object@pen.value.full)
...
setMethod("minseglen", "cpt", function(object)
  object@minseglen)
...
setMethod("cpts", "cpt", function(object)
  object@cpts[-length(object@cpts)])
setMethod("cpts", "cpt.reg", function(object)
  object@cpts[-length(object@cpts)])
...
setMethod("cpts.full", "cpt.range", function(object)
  object@cpts.full)
...
setMethod("cpts.ts", "cpt", function(object)
  index(data.set.ts(object))[cpts(object)])
...
setMethod("ncpts.max", "cpt", function(object)
  object@ncpts.max)
setMethod("ncpts.max", "cpt.reg", function(object)
  object@ncpts.max)
...
setMethod("param.est", "cpt", function(object)
  object@param.est)
setMethod("param.est", "cpt.reg", function(object)
  object@param.est)


setMethod("coef", "cpt", function(object)
  object@param.est)
setMethod("coef", "cpt.reg", function(object)
  object@param.est)
...
setMethod("ncpts", "cpt", function(object)
  length(cpts(object)))
setMethod("ncpts", "cpt.reg", function(object)
  length(cpts(object)))
...
setMethod("seg.len", "cpt", function(object) {
  object@cpts - c(0, object@cpts[-length(object@cpts)])
})
setMethod("seg.len", "cpt.reg", function(object) {
  object@cpts - c(0, object@cpts[-length(object@cpts)])
})
...
setMethod("nseg", "cpt", function(object) {
  ncpts(object) + 1
})
setMethod("nseg", "cpt.reg", function(object) {
  ncpts(object) + 1
})


# replacement functions for slots
setGeneric("data.set<-", function(object, value)
  standardGeneric("data.set<-"))
...
setGeneric("cpttype<-", function(object, value)
  standardGeneric("cpttype<-"))
...
setGeneric("test.stat<-", function(object, value)
  standardGeneric("test.stat<-"))
...
setGeneric("pen.type<-", function(object, value)
  standardGeneric("pen.type<-"))
...
setGeneric("pen.value<-", function(object, value)
  standardGeneric("pen.value<-"))
...
setGeneric("minseglen<-", function(object, value)
  standardGeneric("minseglen<-"))
...
setGeneric("cpts<-", function(object, value)
  standardGeneric("cpts<-"))
...
setGeneric("ncpts.max<-", function(object, value)
  standardGeneric("ncpts.max<-"))
...
setGeneric("param.est<-", function(object, value)
  standardGeneric("param.est<-"))
...
# parameter functions
setGeneric("param", function(object, ...)
...
setMethod("param", "cpt", function(object, shape, ...) {
  ...
  param.scale = function(object, shape) {
    ...
    tmpscale = NULL
    for (j in 1:nseg(object)) {
      tmpscale[j] = (y[(cpts[j + 1] + 1)] - y[(cpts[j] + 1)]) / ((cpts[j + 1] -
                                                                    cpts[j]) * shape)
    }
    return(tmpscale)
  }
  param.trend = function(object) {
    ...
    cptsumstat = matrix(sumstat[object@cpts + 1, ] - sumstat[c(0, cpts(object)) +
                                                               1, ], ncol = 2)
    cptsumstat[, 2] = cptsumstat[, 2] - cptsumstat[, 1] * c(0, cpts(object)) # i.e. creating newx3
    
    thetaS = (2 * cptsumstat[, 1] * (2 * seglen + 1) - 6 * cptsumstat[, 2]) / (2 *
                                                                                 seglen * (2 * seglen + 1) - 3 * seglen * (seglen + 1))
    thetaT = (6 * cptsumstat[, 2]) / ((seglen + 1) * (2 * seglen + 1)) + (thetaS * (1 -
                                                                                      ((3 * seglen) / ((
                                                                                        2 * seglen
                                                                                      ) + 1))))
  ...
  param.meanar = function(object) {
    ...
    data = data.set(object)
    n = length(data) - 1
    sumstat = cbind(cumsum(c(0, data[-1])), cumsum(c(0, data[-(n + 1)])), cumsum(c(0, data[-1] *
                                                                                     data[-(n + 1)])), cumsum(c(0, data[-1] ^ 2)), cumsum(c(0, data[-(n + 1)] ^
                                                                                                                                              2)))
    cptsumstat = matrix(sumstat[object@cpts + 1, ] - sumstat[c(0, cpts(object)) +
                                                               1, ], ncol = 5)
    beta2 = (2 * seglen * cptsumstat[, 3] - cptsumstat[, 1] * cptsumstat[, 2]) /
      (2 * seglen * cptsumstat[, 5] * (1 - cptsumstat[, 2] ^ 2))
  ...
  param.trendar = function(object) {
    ...
    sumstat = cbind(
      cumsum(c(0, data[-1])),
      cumsum(c(0, data[-(n + 1)])),
      cumsum(c(0, data[-1] * data[-(n + 1)])),
      cumsum(c(0, data[-1] * c(1:n))),
      cumsum(c(0, data[-(n + 1)] * c(0:(
        n - 1
      )))),
      cumsum(c(0, data[-1] ^ 2)),
      cumsum(c(0, data[-(n + 1)] ^ 2))
    )
    cptsumstat = matrix(sumstat[object@cpts + 1, ] - sumstat[c(0, cpts(object)) +
                                                               1, ], ncol = 7)
    ...
    betatop = seglen * (seglen - 1) * (
      seglen * (seglen - 1) * cptsumstat[, 3] + 2 * (2 * seglen + 1) * cptsumstat[, 1] *
        (cptsumstat[, 5] - seglen * cptsumstat[, 2]) + 6 * cptsumstat[, 4] * (cptsumstat[, 2] -
                                                                                cptsumstat[, 5])
    )
    betabottom = seglen * (seglen - 1) * cptsumstat[, 7] + 2 * (2 * seglen +
                                                                  1) * cptsumstat[, 2] * (seglen * cptsumstat[, 2] - cptsumstat[, 5]) + 6 *
      cptsumstat[, 5] * (cptsumstat[, 5] - cptsumstat[, 2])
    
    beta = betatop / betabottom
    
    thetajpo = (6 * (seglen + 2) * (cptsumstat[, 4] - beta * cptsumstat[, 5])) /
      ((seglen + 1) * (2 * seglen + 1)) - 2 * (cptsumstat[, 1] - beta * cptsumstat[, 2])
    thetaj = (
      2 * (2 * seglen + 1) * (cptsumstat[, 1] - beta * cptsumstat[, 2]) - 6 *
        (cptsumstat[, 4] - beta * cptsumstat[, 5])
    ) / (seglen - 1)
  ...
  if (cpttype(object) == "mean") {
    ...
    if (test.stat(object) == "Normal") {
      param.est(object) <-
        list(mean = param.mean(object), variance = param.var(object))
    }
    else if (test.stat(object) == "Gamma") {
      param.est(object) <-
        list(scale = param.scale(object, shape = shape),
             shape = shape)
    }
    ...
    stop(
      "Unknown changepoint type, must be 'mean', 'variance', 'mean and variance', 'trend', 'meanar' or 'trendar'."
    )
  }
  return(object)
})
...
setMethod("param", "cpt.range", function(object, ncpts = NA, shape, ...) {
  ...
  param.trendar = function(object, cpts) {
    ...
    n = length(data) - 1
    sumstat = cbind(cumsum(c(0, data[-1])), cumsum(c(0, data[-(n + 1)])), cumsum(c(0, data[-1] *
                                                                                     data[-(n + 1)])), cumsum(c(0, data[-1] * c(1:n))), cumsum(c(0, data[-(n +
                                                                                                                                                             1)] * c(0:(
                                                                                                                                                               n - 1
                                                                                                                                                             )))))
    cptsumstat = matrix(sumstat[object@cpts + 1, ] - sumstat[c(0, cpts(object)) +
  ...
  if (cpttype(object) == "mean") {
    ...
    if (test.stat(object) == "Normal") {
      param.est <-
        list(mean = param.mean(object, cpts),
             variance = param.var(object, cpts))
    }
    else if (test.stat(object) == "Gamma") {
      param.est <-
        list(scale = param.scale(object, cpts, shape = shape),
             shape = shape)
    }
    else if (test.stat(object) == "Exponential") {
...
setMethod("param", "cpt.reg", function(object, shape, ...) {
  param.norm = function(object) {
    ...
    for (j in 1:nseg(object)) {
      ...
      tmpsigma[j] = sum(tmpfit$residuals ^ 2) / (length(tmpfit$residuals) -
                                                   length(tmpfit$coefficients)) ##var(tmpfit$residuals)
    }
...
setMethod("summary", "cpt.range", function(object) {
  ...
  if ((nrow(cpts.full(object)) <= 5) &
      (ncol(cpts.full(object) <= 20))) {
    cat("Range of segmentations:\n")
    print(cpts.full(object))
    cat("\n For penalty values:", pen.value.full(object), "\n")
  }
  else{
    cat(
      "Number of segmentations recorded:",
      nrow(cpts.full(object)),
      " with between ",
      sum(cpts.full(object)[nrow(cpts.full(object)), ] > 0, na.rm = T),
      " and ",
      sum(cpts.full(object)[1, ] > 0, na.rm = T),
      "changepoints.\n Penalty value ranges from:",
      min(pen.value.full(object)),
      " to ",
      max(pen.value.full(object))
    )
  }
})
...
setMethod("show", "cpt", function(object) {
  ...
  cat("       ~~   : S4 class containing",
      length(attributes(object)) - 1,
      "slots with names\n")
  cat("             ", names(attributes(object))[1:(length(attributes(object)) -
                                                      1)], "\n\n")
...
# plot functions
setMethod("plot", "cpt", function(x,
                                  cpt.col = 'red',
                                  cpt.width = 1,
                                  cpt.style = 1,
                                  ...) {
  if (length(param.est(x)) == 0) {
    # i.e. parameter.estimates=FALSE in call
  ...
  if (cpttype(x) == "variance" ||
      cpttype(x) == "nonparametric (empirical_distribution)") {
    abline(
      v = index(data.set.ts(x))[cpts(x)],
      col = cpt.col,
      lwd = cpt.width,
      lty = cpt.style
    )
  }
  else if (cpttype(x) == "mean"  ||
           cpttype(x) == "mean and variance") {
    ...
    slope = rep(param.est(x)$thetaT - param.est(x)$thetaS,
                x@cpts - c(0, cpts(x))) / rep(x@cpts - c(0, cpts(x)), x@cpts - c(0, cpts(x)))
    ...
    stop(
      'Invalid Changepoint Type for plotting.\n Can only plot mean, variance, mean and variance'
    )
  }
})

setMethod("plot", "cpt.range", function(x,
                                        ncpts = NA,
                                        diagnostic = FALSE,
                                        cpt.col = 'red',
                                        cpt.width = 1,
                                        ...
                                        ...) {
  if (diagnostic == TRUE) {
    n.changepoints = apply(cpts.full(x), 1, function(x)
      sum(x > 0, na.rm = TRUE))
  ...
  if (is.na(ncpts)) {
    if (pen.type(x) == "CROPS") {
      stop(
        'CROPS does not supply an optimal set of changepoints, set ncpts to the desired segmentation to plot or use diagnostic=TRUE to identify an appropriate number of changepoints'
      )
    }
    cpts.to.plot = cpts(x)
    param.est = x
  ...
  if (cpttype(x) == "variance") {
    abline(
      v = index(data.set.ts(x))[cpts.to.plot],
      col = cpt.col,
      lwd = cpt.width,
      lty = cpt.style
    )
    ...
    for (i in 1:nseg) {
      segments(
        index(data.set.ts(x))[cpts.to.plot[i] + 1],
        means[i],
        index(data.set.ts(x))[cpts.to.plot[i + 1]],
        means[i],
        col = cpt.col,
...
setMethod("plot", "cpt.reg", function(x,
                                      cpt.col = 'red',
                                      cpt.width = 1,
                                      cpt.style = 1,
                                      ...) {
  ...
  if (test.stat(x) == "Normal") {
    ...
    for (i in 1:nseg(x)) {
      lines((cpts[i] + 1):cpts[i + 1],
            betas[i, ] %*% t(data.set(x)[(cpts[i] + 1):cpts[i + 1], -1]),
            col = cpt.col,
            lwd = cpt.width,
            lty = cpt.style
      )
  ...
  if (test.stat(object) == "Normal") {
    if (cpttype(object) == "mean") {
      ...
      if (pen.type(object) == "MBIC") {
        like = c(like, like + (nseg(object) - 2) * pen.value(object) + sum(log(cpts[-1] -
                                                                                 cpts[-(nseg(object) + 1)])))
      } else{
      ...
      rss = c(0, cumsum((
        data.set(object) - param.est(object)$mean
      ) ^ 2))
      cpts = c(0, object@cpts)
      n = length(data.set(object))
      seglen = seg.len(object)
      sigmas = (rss[cpts[-1] + 1] - rss[cpts[-length(cpts)] + 1]) /
        seglen
      ...
      slope = rep(param.est(object)$thetaT - param.est(object)$thetaS,
                  object@cpts - c(0, cpts(object))) / rep(object@cpts - c(0, cpts(object)), object@cpts -
                                                            c(0, cpts(object)))
      ...
      slope = rep(param.est(object)$thetajpo - param.est(object)$thetaj,
                  seglen) / rep(seglen, seglen)
    ...
    if (cpttype(object) != "mean and variance") {
      ...
      tmplike = 0
      for (j in 1:nseg(object)) {
        tmplike = tmplike + mll.meanvarg(y[cpts[j + 1] + 1] - y[cpts[j] + 1], cpts[j +
                                                                                     1] - cpts[j], shape)
      }
      if (pen.type(object) == "MBIC") {
        like = c(tmplike,
                 tmplike + (nseg(object) - 2) * pen.value(object) + sum(log(seg.len(object))))
      } else{
    ...
    if (cpttype(object) != "mean and variance") {
      stop(
        "Unknown changepoint type for test.stat='Exponential', must be 'mean and variance'"
      )
    }
    else{
      warning("Not changed to be -2*logLik")
      ...
      tmplike = 0
      for (j in 1:nseg(object)) {
        tmplike = tmplike + mll.meanvare(y[cpts[j + 1] + 1] - y[cpts[j] + 1], cpts[j +
                                                                                     1] - cpts[j])
      }
      if (pen.type(object) == "MBIC") {
        like = c(tmplike,
    ...
    if (cpttype(object) != "mean and variance") {
      ...
      tmplike = 0
      for (j in 1:nseg(object)) {
        tmplike = tmplike + mll.meanvarp(y[cpts[j + 1] + 1] - y[cpts[j] + 1], cpts[j +
                                                                                     1] - cpts[j])
      }
      if (pen.type(object) == "MBIC") {
        like = c(tmplike,
...
setMethod("logLik", "cpt.range", function(object, ncpts = NA) {
  ...
  if (is.na(ncpts)) {
    if (pen.type(object) == "CROPS") {
      stop(
        'CROPS does not supply an optimal set of changepoints, set ncpts argument to the desired segmentation to plot or use diagnostic=TRUE to identify an appropriate number of changepoints'
      )
    }
    cpts = c(0, object@cpts)
  ...
  if (test.stat(object) == "Normal") {
    if (cpttype(object) == "mean") {
      ...
      tmplike = 0
      for (j in 1:nseg) {
        tmplike = tmplike + mll.mean(y2[cpts[j + 1] + 1] - y2[cpts[j] + 1], y[cpts[j +
                                                                                     1] + 1] - y[cpts[j] + 1], cpts[j + 1] - cpts[j])
      }
      ...
      tmplike = 0
      for (j in 1:nseg) {
        tmplike = tmplike + mll.var(y2[cpts[j + 1] + 1] - y2[cpts[j] + 1], cpts[j +
                                                                                  1] - cpts[j])
      }
      if (pen.type(object) == "MBIC") {
        like = c(tmplike, tmplike + (nseg - 2) * pen.value + sum(log(seg.len(object))))
      ...
      tmplike = 0
      for (j in 1:nseg) {
        tmplike = tmplike + mll.meanvar(y2[cpts[j + 1] + 1] - y2[cpts[j] + 1], y[cpts[j +
                                                                                        1] + 1] - y[cpts[j] + 1], cpts[j + 1] - cpts[j])
      }
      if (pen.type(object) == "MBIC") {
        like = c(tmplike, tmplike + (nseg - 2) * pen.value + sum(log(seg.len(object))))
...
setMethod("logLik", "cpt.reg", function(object) {
  ...
  if (test.stat(object) == "Normal") {
    ...
    for (i in 1:length(seglen)) {
      rss[i] = sum((data[(cpts[i] + 1):cpts[i + 1], 1] - data[(cpts[i] + 1):cpts[i +
                                                                                   1], -1] %*% beta[i, ]) ^ 2)
    }
...
setGeneric("likelihood", function(object)
