...
Delta=1
cpt.s = c(rep(0,floor(n/4)), rep(2*Delta, floor(n/4)), rep(Delta, floor(n/4)),rep(0,n-3*floor(n/4))) ##3 Alternating Cpts
regmeandata=cpt.s+cos(2*pi*(1:n -6)/12)+rnorm(n)
regmeandata=cbind(regmeandata,indicator,seasonal)
regtrenddata=0.2*cpt.s*1:n+cos(2*pi*(1:n -6)/12)+rnorm(n)
regtrenddata=cbind(regtrenddata,indicator,trend,seasonal)
regseasonaldata=0.1*1:n+cpt.s*cos(2*pi*(1:n -6)/12)+rnorm(n)
regseasonaldata=cbind(regseasonaldata,indicator,trend,seasonal)
nochangedata=0.2*1:n+cos(2*pi*(1:n -6)/12)+rnorm(n)
nochangedata=cbind(nochangedata,indicator,trend,seasonal)

constantdata <- matrix(rep(1, 200),ncol=2)
shortdata <- matrix(c(2,1),ncol=2)
negativedata <- matrix(c(jitter(rep(-100, 100)),rep(1,100)),ncol=2)

characterdata <-matrix(c(rep("ert", 100),rep(1,100)),ncol=2)

#NAdata - creates 10 random NA within singmeandata
NAdata <- regmeandata
rn <- sample(1:nrow(regmeandata), 10, replace=F)
NAdata[rn]=NA
###################

data <- list(regmeandata,regtrenddata, regseasonaldata, nochangedata, constantdata, NAdata, shortdata, negativedata, characterdata)
...
penalties <- c("SIC", "BIC", "AIC", "Hannan-Quinn", "Asymptotic", "Manual", "MBIC", "CROPS")
    ...
    test_that(paste0("Test #",t," :data=", d, "penalty=",penalties[p],", method=",methods[m],",class=",cl,", param=",pe,", penvalue=",manpenval[[npv]],", test.stat=",testStats[ts]) ,{
      ...
      if(is.numeric(manpenval[[npv]]) == FALSE){
        ...
        if(class(texttest)=='try-error'){
                 expect_that(cpt.reg(data=data[[d]], penalty=penalties[p], pen.value=manpenval[[npv]], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe), throws_error("Your manual penalty cannot be evaluated"))
          t = t + 1
        }
      }
            if(manpenval[[npv]] < 0){
              expect_that(cpt.reg(data=data[[d]], penalty=penalties[p], pen.value=manpenval[[npv]], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe), throws_error('pen.value cannot be negative, please change your penalty value'))
            }
  ...
  for(apv in 1:length(asympenval)){
    ...
    test_that(paste0("Test #",t," :data=", d, "penalty=",penalties[p],", pen.value=", asympenval[[apv]],", method=",methods[m],",class=",cl,", param=",pe,", penvalue=",asympenval[[apv]],", test.stat=",testStats[ts]), {
      if(testStats[ts] == "CUSUM"){
        ...
        expect_that(cpt.reg(data=data[[d]], penalty=penalties[p], pen.value=asympenval[[apv]], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe), throws_error("Asymptotic penalty values must be > 0 and <= 1"))
      }
      else if(methods[m] == "PELT" || methods[m] == "BinSeg"){
          expect_warning(cpt.reg(data=data[[d]], penalty=penalties[p], pen.value=asympenval[[apv]], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe))
      }
      else{
        x <- cpt.reg(data=data[[d]], penalty=penalties[p], pen.value=asympenval[[apv]], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe)
        expect_that(2+2, equals(4))
      }
    })
    t = t+1
...
checkCROPS <- function(){
  ...
  expect_that(cpt.reg(data=data[[d]], method=methods[m],penalty=penalties[p], pen.value=cropspenval[[1]], test.stat=testStats[[ts]], class=cl, param.estimates=pe), throws_error('CROPS has not yet been implemented for cpt.reg'))
  ...
  #         expect_that(cpt.reg(data=data[[d]], method=methods[m],penalty=penalties[p], pen.value=cropspenval[[cr]], test.stat=testStats[[ts]], class=cl, param.estimates=pe), throws_error('For CROPS, pen.value must be supplied'))
  #       }else if(length(cropspenval[[cr]]) != 2 ){
  #         expect_that(cpt.reg(data=data[[d]], method=methods[m],penalty=penalties[p], pen.value=cropspenval[[cr]], test.stat=testStats[[ts]],class=cl, param.estimates=pe), throws_error('The length of pen.value must be 2'))
  #
  #
  #       }else{
  ...
  #           x <- cpt.reg(data=data[[d]], method=methods[m],penalty=penalties[p], pen.value=cropspenval[[cr]], test.stat=testStats[[ts]], class=cl, param.estimates=pe)
  #
  #
  #           if(cl == TRUE){
  ...
  #
  #         }else{
  #           expect_that(cpt.reg(data=data[[d]], method=methods[m],penalty=penalties[p], pen.value=cropspenval[[cr]], test.stat=testStats[[ts]], class=cl, param.estimates=pe), throws_error('Only Normal, Exponential, Gamma and Poisson are valid test statistics'))
  #         }
  #       }
  #     })
  #     t=t+1
...
checkOtherPenalties <- function(methodLog){
  ...
  test_that(paste0("Test #",t," :data=", d, "penalty=",penalties[p],", method=",methods[m],",class=",cl,", param=",pe,", test.stat=",testStats[ts]), {
    x <- cpt.reg(data=data[[d]], penalty=penalties[p], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe)
...
for(d in 1:length(data)){
  if(is.element(NA, data[[d]])){
    test_that(paste0("Test #",t," :data=",d), {
      expect_that(cpt.reg(data=data[[d]]),throws_error("Missing value: NA is not allowed in the data as changepoint methods are only sensible for regularly spaced data."))
      t = t + 1
    })

  }else if(nrow(data[[d]]) < 2){
    test_that(paste0("Test #",t," :data=",d), {
      expect_that(cpt.reg(data=data[[d]]),throws_error())
      t = t + 1
    })
  }else if(is.numeric(data[[d]]) == FALSE){
    test_that(paste0("Test #",t," :data=",d), {
      expect_that(cpt.reg(data=data[[d]]),throws_error("Only numeric data allowed"))
      t = t + 1
    })
  }
  else{
                ...
                if(methods[m] == "BinSeg" || methods[m] == "SegNeigh"){
                  for(v in 1:length(QValues)){
                    ...
                    if(penalties[p] == "Manual" ){
                      ...
                      if(testStats[ts]=="CUSUM"||testStats[ts]=="CSS"){
                        test_that(paste0("Test #",t," :data=", d, "penalty=",penalties[p],", method=",methods[m],",class=",cl,", param=",pe,", test.stat=",testStats[ts],"QVal=",Qv),expect_error(cpt.reg(data=data[[d]],penalty=penalties[p],method=methods[m],test.stat=testStats[ts],class=cl,param.estimates=pe)))
                      }else{
                  ...
                  if(methods[m] == "PELT" && testStats[ts] == "CUSUM"){
                    test_that(paste0("Test #",t," :data=", d, "penalty=",penalties[p],", method=",methods[m],",class=",cl,", param=",pe,", test.stat=",testStats[ts],"QVal=",Qv), {
                      expect_that(cpt.reg(data=data[[d]], penalty=penalties[p], method=methods[m], test.stat=testStats[ts], class=cl, param.estimates=pe), throws_error("Invalid Method, must be AMOC, SegNeigh or BinSeg"))
                    })
                    t = t + 1
                  }else{
                    if(penalties[p] == "Manual" ){
